...
public class Login extends javax.swing.JPanel {
    ...
    if (sqlite.authenticateUser(username, password)) { // Authenticate first
        int userRole = sqlite.getUserRole(username); // Get role after authentication
        ...
        if (userRole == 3) { // Staff 
          StaffHome staffHome = new StaffHome();
            frame.setContentPane(staffHome);
            frame.revalidate();
            frame.repaint();
            staffHome.init(sqlite); 
         staffHome.setVisible(true);
            ...
            frame.revalidate();
            frame.repaint();
              managerHome.init(sqlite); 
        } else if (userRole == 5) { // Admin
            AdminHome adminHome = new AdminHome();
            frame.setContentPane(adminHome);
            frame.revalidate();
            frame.repaint();
            adminHome.init(sqlite); 
        } else {
            JOptionPane.showMessageDialog(this, "Access Denied.", "Access Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
